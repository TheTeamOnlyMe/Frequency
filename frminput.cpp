	...
	if (event->type() == QEvent::MouseButtonPress) {
		...
		if (mouseEvent->button() == Qt::LeftButton) {
			if (obj == ui->labCh0) {
				...
				{
					if (!obj->property("noinput").toBool() &&!obj->property("nokeyboard").toBool()&& objName != "frmMainWindow"
...
void frmInput::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
	...
	if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
		...
		if (nowWidget->property("nokeyboard").toBool()) {
			QTimer::singleShot(0, this, SLOT(hide()));
			return;
		}
		frmNum::Instance()->setDefualt();
		isFirst = false;
