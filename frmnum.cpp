...
frmNum::~frmNum()
{
	...
	if (timerPress !=0)
	{
		disconnect(timerPress, SIGNAL(timeout()), this, SLOT(reClicked()));
		delete timerPress ;timerPress =NULL;
	}

	QList<QPushButton *> btn = this->findChildren<QPushButton *>();
	foreach (QPushButton * b, btn) {
		disconnect(b, SIGNAL(clicked()), this, SLOT(btn_clicked()));
		delete b;
	}
...
void frmNum::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
	...
	if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
		...
		if (nowWidget->property("nonum").toBool()) {
			QTimer::singleShot(0, this, SLOT(hide()));
			return;
		}
		frmInput::Instance()->setDefualt();
		isFirst = false;
		currentEditTypeName =nowWidget->objectName();
	...
	if (event->type() == QEvent::MouseButtonPress) {
		...
		if (currentEditType != "") {
			if (obj != ui->btnClose) {
				...
				if (currentWidget == obj||currentWidget ==0)//
				{
					if (!obj->property("noinput").toBool() &&!obj->property("nonum").toBool()&& objName != "frmMainWindow"
						&& objName != "frmInputWindow" && objName != "qt_edit_menu"&&objName ==currentEditTypeName) {
							ShowPanel();
					}
				}
				else {
					if(objName == "qt_spinbox_lineedit") 
						ShowPanel();
				}
			}
...
void frmNum::insertValue(QString value)
{
	if (currentEditType == "QLineEdit") {
		...
		if (currentWidget->inherits("QDateTimeEdit"))
		{
			QDateTimeEdit* tempwidget =(QDateTimeEdit*)currentWidget;
			if (value =="6")
			{
				QKeyEvent rightKey(QEvent::KeyPress, Qt::Key_Right, Qt::NoModifier);
				QCoreApplication::sendEvent(tempwidget, &rightKey);
			}
			else if (value =="4")
			{
				QKeyEvent leftKey(QEvent::KeyPress, Qt::Key_Left, Qt::NoModifier);
				QCoreApplication::sendEvent(tempwidget, &leftKey);
			}
			else if (value =="8")
			{
				QKeyEvent upKey(QEvent::KeyPress, Qt::Key_Up, Qt::NoModifier);
				QCoreApplication::sendEvent(tempwidget, &upKey);
			}
			else if (value =="2")
			{
				QKeyEvent downKey(QEvent::KeyPress, Qt::Key_Down, Qt::NoModifier);
				QCoreApplication::sendEvent(tempwidget, &downKey);
			}
		}
		else{
